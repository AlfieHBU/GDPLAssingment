...
public class ProjectileMovement : MonoBehaviour
{
    ...
    private IEnumerator DecreaseProjectileCountAfterDelay(float delay) 
    {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(Fire) && currentProjectileCount < 1 && currentAmmo > 0)
        {
            ...
            StartCoroutine(DestroyProjectileAfterUse(Projectile));
    ...
    private IEnumerator DestroyProjectileAfterUse(GameObject projectile) 
    {
        yield return new WaitUntil(() => 
            projectile.transform.position.y < -10f || projectile.GetComponent<Rigidbody>().IsSleeping());
